...
export async function createRecord({
  ...
  try {
    ...
    if (updateError) {
      throw new Error(
        `Failed to update record with URLs: ${updateError.message}`,
      );
    }
...
export async function getRecord(userId: string, recordId: string) {
  try {
    const { data: record, error } = await (supabaseAdmin as any)
      .from("records")
      .select(
        "id, user_id, form_data, generated_on, valid_upto, public_token, status, qr_code_url, pdf_url, total_scans, created_at",
      )
      .eq("id", recordId)
      .eq("user_id", userId)
      .single();
...
/**
 * Get record by ID (requires record ID, not public token)
 * Used for dashboard views where user already has access
 */
...
export async function getPublicRecord(publicToken: string) {
  try {
    const { data: record, error } = await (supabaseAdmin as any)
      .from("records")
      .select(
        "id, form_data, generated_on, valid_upto, status, total_scans, qr_code_url, pdf_url",
      )
